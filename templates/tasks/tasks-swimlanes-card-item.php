<div class="dropzone rounded" ondrop="drop(event);iterateSwimlanes();" ondragover="allowDrop(event)" ondragleave="clearDrop(event)"> &nbsp; </div>
...
<div class="card card-item card-item draggable shadow-sm"
    ...
    <div class="hidden">
        <input type="hidden" class="card-item-status" name="tasks[<?=$taskTodo['id'];?>]" value="<?=$taskTodo['state'];?>" >
    </div>
</div>
